...
inputFile1 = LOAD 'hdfs:///user/lakshmi/episodeIV_dialogues.txt' using PigStorage('\t') AS (name,line);
inputFile2 = LOAD 'hdfs:///user/lakshmi/episodeV_dialogues.txt' using PigStorage('\t') AS (name,line);
inputFile3 = LOAD 'hdfs:///user/lakshmi/episodeVI_dialogues.txt' using PigStorage('\t') AS (name,line);
...
STORE Sorted_Output INTO 'hdfs:///user/lakshmi/Output_Characters';
